...
/*! tooltipster v4.2.5 */!function (a, b) {
    ...
    function b(a) {
        ...
        } };var j = navigator.userAgent.toLowerCase();-1 != j.indexOf("msie") ? h.IE = parseInt(j.split("msie")[1]) : -1 !== j.toLowerCase().indexOf("trident") && -1 !== j.indexOf(" rv:11") ? h.IE = 11 : -1 != j.toLowerCase().indexOf("edge/") && (h.IE = parseInt(j.toLowerCase().split("edge/")[1]));var k = "tooltipster.sideTip";return a.tooltipster._plugin({
        name: k, instance: {
...
!function (root, name, definition) {
    if (typeof module != 'undefined' && module.exports) module.exports = definition();else if (typeof define == 'function' && define.amd) define(name, definition);else {
...
$(function () {
    app.footer.html("<p>\xA9 " + new Date().getFullYear() + ' Frederik Nielsen</p>');
...
app.toggleFullScreen = function (element) {
    if (app.fullscreen) exitFullScreen();else requestFullScreen(element);
};
var app = app || {};
...
$(function () {
    ...
    app.addValidation(app.authentication.find('.register > form'), {
        register_username: {
            required: true,
            minlength: 2
        },
    ...
    app.addValidation(app.authentication.find('.login > form'), {
        username: {
            required: true
        },
        password: {
...
(function (l) {
    if (l.search) {
        ...
        if (app.url.p !== undefined) {
            window.history.replaceState(null, null, l.pathname.slice(0, -1) + app.url.p + (app.url.q ? '?' + app.url.q : '') + l.hash);
        }
    }
})(window.location);
...
app.applyNavigation = function (id, value, set) {
    if (set) {
        var entry = {
            ...
            exists = $.grep(app.navigation, function (e) {
            return e.name === name;
        });
        if (exists.length === 0) {
            // not found
...
app.applySettings = function (id, name, type, value, set) {
    if (set) {
        var entry = {
            ...
            exists = $.grep(app.settings, function (e) {
            return e.name === name;
        });
        if (exists.length === 0) {
            // not found
    ...
    if (type === 'checkbox' || type === "radio") {
        ...
        if (name === 'theme') {
            ...
            for (var _i = 1; _i < split2.length; _i++) {
                href.push('.' + split2[_i]);
            }
            href = href.join("");
...
$(function () {
    app.right.find('> .content > div').load(app.host + 'ajax/layout/settings.html', function () {
        ...
        $(this).on('change', 'input[type=checkbox], input[type=radio]', function () {
            ...
            if (id === 'left-shrink' || id === 'right-shrink' || id === 'left-push' || id === 'right-push' || id === 'left-overlay' || id === 'right-overlay') {
                app.setHtmlScroll();
            }
        });
        app.responsiveHeader();
...
$(function () {
    app.body.on("keydown", function (e) {
        ...
        if (app.isLoading()) {
            if (e.which === 9 || e.ctrlKey && e.keyCode === 65) {
                // tab ||  ctrl + a
                e.preventDefault();
            }
        } else {
            if (e.which === 37 && !app.isFocus() && !app.isModal()) {
                // left
                if (app.isAsideLeft()) {
            ...
            } else if (e.which === 39 && !app.isFocus() && !app.isModal()) {
                // right
                if (app.isAsideRight()) {
...
app.checkRb = function (elements) {
    function setRb(element) {
        var image = element.attr('data-rb-image'),
        ...
        if (image !== undefined && image.length && filetype !== undefined && filetype.length && sizesWidth !== undefined && sizesWidth.length && aspectRatio !== undefined && aspectRatio.length) {
            if ((filetype === 'jpg' || filetype === 'jpeg' || filetype === 'png') && (aspectRatio === '21by9' || aspectRatio === '16by9' || aspectRatio === '4by3' || aspectRatio === '1by1')) {
                var getHeightInPercentage = function getHeightInPercentage(num, amount) {
                    return num * 100 / amount;
                };

                var getHeightInPixels = function getHeightInPixels(num, amount) {
                    return num * amount / 100;
                };

                sizesWidth = sizesWidth.replace(/\s/g, '').split(',').sort(function (a, b) {
                    return a - b;
                });
                var goalWidth = element.width(),
                    goalHeight = element.height(),
                    closestWidth = void 0,
                    closestHeight = void 0,
                    heightPercentage = void 0;
                ...
                $.each(sizesWidth, function (index) {
                    ...
                    if (closestWidth === undefined || width < goalWidth || closestWidth < goalWidth || closestHeight === undefined || height < goalHeight || closestHeight < goalHeight) {
                        closestWidth = width;
...
app.enableFocus = function () {
    var component = app.Focus.find('> .component > div');
...
app.enableTTS = function () {
    if (bowser.desktop) {
        var snapSelectionToWord = function snapSelectionToWord() {
            var selection = window.getSelection();
...
app.checkDropdown = function (dropdowns) {
    dropdowns.each(function () {
        ...
        html.push('<div class="dropdown' + ($this.hasClass('not-first') ? ' not-first' : '') + ($this.hasClass('nowrap') ? ' nowrap' : '') + ($this.hasClass('check') ? ' check' : '') + ($this.hasClass('ellipsis') ? ' ellipsis' : '') + ($this.hasClass('align-left') ? ' align-left' : '') + ($this.hasClass('align-right') ? ' align-right' : '') + ($this.hasClass('direction-up') ? ' direction-up' : '') + '"' + (width !== undefined ? ' style="width:' + width + 'px"' : '') + '> ');
...
app.checkAccordion = function (elements) {
    elements.on("click", ".headline", function () {
        ...
        if (content.hasClass('open')) {
            ...
            content.addClass("open").slideToggle("800").parents('.accordion').find(".content.open").not(content).removeClass('open').slideUp("800");
        }
    });
};
var app = app || {};
...
app.checkMap = function (maps) {
    ...
    if (maps.length) {
        var checkGoogle = function checkGoogle() {
            if (google !== undefined) {
                maps.each(function () {
                    var $this = $(this),
...
$(function () {
    app.body.on('click', '.show-popup', function () {
        ...
        if (title !== undefined) {
            ...
            _alert.push('<div class="alert theme-' + theme + '">');
            _alert.push('<div><p>' + title + '</p></div>');
            _alert.push('<button class="close" aria-label="Close popup"><svg focusable="false"><use xlink:href="#svg-close"></use></svg></button>');
            _alert.push('</div>');
            _alert = _alert.join('');
    ...
    if (tables.length) {
        $.getScript('dist/js/datatables.min.js', function () {
            ...
            function table_header_input(instance) {
                ...
                var elements = jQuery.grep(columns, function (e) {
                    var column = $(e);
                ...
                instance.columns().every(function () {
                    ...
                    if (column.hasClass('dropdown')) {
                        var select = $('<select class="dropdown align-left nowrap"><option value=""></option></select>').appendTo(column.find('> div > div:last-child')).on('change', function () {
                        ...
                        var input = $('<input type="text" />').appendTo(column.find('> div > div:last-child')).on('keyup change', function () {
                            instance.column(index).search(this.value).draw();
                        });
            ...
            function table_header_buttons(wrapper, header) {
                var container = wrapper.find('.dt-buttons');
            ...
            function paginateFix(paginate) {
                var span = paginate.children('span'),
            ...
            function table_footer_paginate(wrapper, footer, paginate) {
                ...
                var container = footer.find('.dataTables_paginate_container');
            ...
            tables.each(function () {
                ...
                $this.DataTable({
                    ...
                    initComplete: function initComplete(settings, json) {
                        ...
                        var table = table_content(wrapper, content);
                        ...
                        $this.on('responsive-resize.dt', function (e, datatable, columns) {
                            ...
                            var elements = table.find('tbody tr:not(.child) > *:first-child:not(.dataTables_empty)');
